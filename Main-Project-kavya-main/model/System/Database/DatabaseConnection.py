...
class DatabaseConnection:
    def __init__(self):
        self.conn = None
        self.cursor = None
        print("init of databaseconnection")

    def connect(self):
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        db_path = os.path.join(BASE_DIR, "Argus_DB.db")
        print("inside database connection function")
        self.conn = sqlite3.connect(db_path)
        print("connection done of argusdb")
    ...
    def selectCrashFrames(self):
        ...
        # query = "SELECT * from client_crashed_frames Where from_no < 5"
